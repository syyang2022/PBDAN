...
def CrossEntropyLoss(label, predict_prob):
    """
    计算交叉熵损失
    :param label: 标签，可以是类别索引（一维）或one-hot编码（二维）
    :param predict_prob: 预测概率，二维张量 (batch_size, num_classes)
    :return: 交叉熵损失
    """
    # 处理标签维度
    if label.dim() == 1:
        # 一维标签：直接使用类别索引
        N = label.size(0)
        C = predict_prob.size(1)
    elif label.dim() == 2:
        # 二维标签：one-hot编码，转换为类别索引
        N, C = label.size()
        label = torch.argmax(label, dim=1)
    else:
        raise ValueError(f"标签维度错误: {label.dim()}")

    # 使用PyTorch内置的交叉熵损失
    criterion = torch.nn.CrossEntropyLoss()
    loss = criterion(predict_prob, label)
