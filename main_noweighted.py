...
# 创建结果目录
os.makedirs("./results_noweighted", exist_ok=True)
...
class DomainDistanceMonitor:
    ...
    实时监控三个域之间的距离变化
    """
    def __init__(self, log_dir='./results_noweighted/domain_monitor'):
        ...
        if bridge_loader is not None:
            for (i, ((im_source, label_source), (im_target, label_target), (im_bridge, label_bridge))) in enumerate(
                ...
                adv_loss += BCELossForMultiClassification(
                    label=torch.zeros_like(domain_prob_discriminator_1_target),  # 注意：这里标签应该是0（目标域）
                    predict_prob=domain_prob_discriminator_1_target  # 预测概率，不需要1-变换
                )
...
plt.tight_layout()
plt.savefig('./results_noweighted/confusion_matrix_wavelet.png', dpi=300, bbox_inches='tight')
plt.close()
...
def visualize_features(net, target_loader, label_names, device=device):
    ...
    plt.savefig('./results_noweighted/feature_visualization_wavelet.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def evaluate_bridge_domain_performance(net, source_loader, bridge_loader, target_loader, device=device):
    ...
    if bridge_loader is not None:
        ...
        plt.tight_layout()
        plt.savefig('./results_noweighted/bridge_domain_effect_wavelet.png', dpi=300, bbox_inches='tight')
        plt.close()
