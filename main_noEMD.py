...
# 创建结果目录
os.makedirs("./results_noEMD", exist_ok=True)
...
class DomainDistanceMonitor:
    ...
    实时监控三个域之间的距离变化
    """
    def __init__(self, log_dir='./results_noEMD/domain_monitor'):
...
plt.tight_layout()
plt.savefig('./results_noEMD/confusion_matrix_wavelet.png', dpi=300, bbox_inches='tight')
plt.close()
...
def visualize_features(net, target_loader, label_names, device=device):
    ...
    plt.savefig('./results_noEMD/feature_visualization_wavelet.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def evaluate_bridge_domain_performance(net, source_loader, bridge_loader, target_loader, device=device):
    ...
    if bridge_loader is not None:
        ...
        plt.tight_layout()
        plt.savefig('./results_noEMD/bridge_domain_effect_wavelet.png', dpi=300, bbox_inches='tight')
        plt.close()
