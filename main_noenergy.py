...
# 创建结果目录
os.makedirs("./results_noenergy", exist_ok=True)
...
def get_dat(data_num):
    if data_num == 1:
        ...
        # 新增：加载桥梁域数据
        try:
            F_bridge = np.load("./bearingset/data_thirdspace/T1_bridge_image.npy").transpose((0, 3, 1, 2)) / 255
            Y_bridge = np.load("./bearingset/data_thirdspace/T1_bridge_label.npy")
            print(f"成功加载桥梁域数据: {F_bridge.shape[0]} 个样本")
        except FileNotFoundError:
            print("警告: 桥梁域数据文件未找到，跳过桥梁域")
            F_bridge = None
            Y_bridge = None
    ...
    if data_num == 2:
        ...
        # 加载桥梁域数据
        try:
            F_bridge = np.load("./bearingset/data_thirdspace/T2_bridge_image.npy").transpose((0, 3, 1, 2)) / 255
            Y_bridge = np.load("./bearingset/data_thirdspace/T2_bridge_label.npy")
            print(f"成功加载桥梁域数据: {F_bridge.shape[0]} 个样本")
        except FileNotFoundError:
            print("警告: 桥梁域数据文件未找到，跳过桥梁域")
            F_bridge = None
    ...
    if data_num == 3:
        ...
        # 加载桥梁域数据
        try:
            F_bridge = np.load("./bearingset/data_thirdspace/T3_bridge_image.npy").transpose((0, 3, 1, 2)) / 255
            Y_bridge = np.load("./bearingset/data_thirdspace/T3_bridge_label.npy")
            print(f"成功加载桥梁域数据: {F_bridge.shape[0]} 个样本")
        except FileNotFoundError:
            print("警告: 桥梁域数据文件未找到，跳过桥梁域")
            F_bridge = None
    ...
    if data_num == 4:
        ...
        # 加载桥梁域数据
        try:
            F_bridge = np.load("./bearingset/data_thirdspace/T4_bridge_image.npy").transpose((0, 3, 1, 2)) / 255
            Y_bridge = np.load("./bearingset/data_thirdspace/T4_bridge_label.npy")
            print(f"成功加载桥梁域数据: {F_bridge.shape[0]} 个样本")
        except FileNotFoundError:
            print("警告: 桥梁域数据文件未找到，跳过桥梁域")
            F_bridge = None
    ...
    if data_num == 5:
        ...
        # 加载桥梁域数据
        try:
            F_bridge = np.load("./gearset/datagearset_thirdspace/T1_bridge_image.npy").transpose((0, 3, 1, 2)) / 255
            Y_bridge = np.load("./gearset/datagearset_thirdspace/T1_bridge_label.npy")
            print(f"任务1 - 成功加载桥梁域数据: {F_bridge.shape[0]} 个样本")
        except FileNotFoundError:
            print("警告: 任务1桥梁域数据文件未找到，跳过桥梁域")
            F_bridge = None
            Y_bridge = None
    ...
    if data_num == 6:
        ...
        # 加载桥梁域数据
        try:
            F_bridge = np.load("./gearset/datagearset_thirdspace/T2_bridge_image.npy").transpose((0, 3, 1, 2)) / 255
            Y_bridge = np.load("./gearset/datagearset_thirdspace/T2_bridge_label.npy")
            print(f"任务2 - 成功加载桥梁域数据: {F_bridge.shape[0]} 个样本")
        except FileNotFoundError:
            print("警告: 任务2桥梁域数据文件未找到，跳过桥梁域")
            F_bridge = None
            Y_bridge = None
    ...
    if data_num == 7:
        ...
        # 加载桥梁域数据
        try:
            F_bridge = np.load("./gearset/datagearset_thirdspace/T3_bridge_image.npy").transpose((0, 3, 1, 2)) / 255
            Y_bridge = np.load("./gearset/datagearset_thirdspace/T3_bridge_label.npy")
            print(f"任务3 - 成功加载桥梁域数据: {F_bridge.shape[0]} 个样本")
        except FileNotFoundError:
            print("警告: 任务3桥梁域数据文件未找到，跳过桥梁域")
            F_bridge = None
            Y_bridge = None

        # 任务3: 1,3,4 → 1,2,3,4,5,6
        Label_Name = ["1-N", "3-BF&SI", "4-G&E", "Unknown"]
    ...
    # 处理桥梁域数据
    if F_bridge is not None and Y_bridge is not None:
        F_bridge = torch.from_numpy(F_bridge.astype(np.float32))
        Y_bridge = torch.from_numpy(Y_bridge.astype(np.float32))
        data_bridge = Data.TensorDataset(F_bridge, Y_bridge)
        return data_0, data_1, data_bridge, F_0, F_1, Y_0, Y_1, Label_Name
    else:
...
class AdaptiveBridgeWeight:
    def __init__(self, initial_weight=0.1, max_weight=0.5, warmup_steps=100,
                 decay_start=500, decay_rate=0.99):
    ...
    def get_weight(self):
        ...
        if self.current_step < self.warmup_steps:
            # 预热阶段：线性增加
            weight = self.initial_weight + (self.max_weight - self.initial_weight) * (
            ...
            # 衰减阶段：指数衰减
            decay_steps = self.current_step - self.decay_start
            weight = self.max_weight * (self.decay_rate ** (decay_steps / 100))
...
class DomainDistanceMonitor:
    """
    域间距离监控器
    实时监控三个域之间的距离变化
    """
    def __init__(self, log_dir='./results_noenergy/domain_monitor'):
    ...
    def compute_distances(self, net, source_loader, bridge_loader, target_loader, epoch=None):
        """
        计算三个域之间的特征距离
        """
        net.eval()
        ...
        # 提取特征
        src_features, bridge_features, tgt_features = [], [], []

        with torch.no_grad():
            # 源域特征
            for im, _ in source_loader:
                im = im.to(device)
                f, _, _ = net(im)
                src_features.append(f.cpu().numpy())

            # 桥梁域特征
            if bridge_loader is not None:
                for im, _ in bridge_loader:
                    im = im.to(device)
                    f, _, _ = net(im)
                    bridge_features.append(f.cpu().numpy())

            # 目标域特征
            for im, _ in target_loader:
                im = im.to(device)
                f, _, _ = net(im)
                tgt_features.append(f.cpu().numpy())

        # 合并特征
        src_features = np.concatenate(src_features, axis=0) if src_features else None
        bridge_features = np.concatenate(bridge_features, axis=0) if bridge_features else None
        tgt_features = np.concatenate(tgt_features, axis=0) if tgt_features else None

        # 计算距离
        distances = {}

        if src_features is not None and tgt_features is not None:
            # 源域-目标域距离
            src_center = np.mean(src_features, axis=0)
            tgt_center = np.mean(tgt_features, axis=0)
            src_tgt_dist = np.linalg.norm(src_center - tgt_center)
            distances['src_tgt'] = src_tgt_dist

            if bridge_features is not None:
                # 源域-桥梁域距离
                bridge_center = np.mean(bridge_features, axis=0)
                src_bridge_dist = np.linalg.norm(src_center - bridge_center)
                distances['src_bridge'] = src_bridge_dist

                # 桥梁域-目标域距离
                bridge_tgt_dist = np.linalg.norm(bridge_center - tgt_center)
                distances['bridge_tgt'] = bridge_tgt_dist

                # 桥梁效率（值越大说明桥梁域作用越好）
                bridge_efficiency = src_tgt_dist / (src_bridge_dist + bridge_tgt_dist + 1e-8)
                distances['efficiency'] = bridge_efficiency

                # 域间差距减少量
                gap_reduction = src_tgt_dist - (src_bridge_dist + bridge_tgt_dist)
                distances['gap_reduction'] = gap_reduction
...
def step_1_enhanced_complete(source_loader, target_loader, bridge_loader,
    ...
    bridge_loss_fn = BridgeDomainLoss(
        lambda_mmd=0.2,
        lambda_center=0.1,
        lambda_class_aware=0.3,  # 新增：类别感知MMD的权重
        mmd_type='both'  # 或者 'both' 表示同时使用标准MMD和类别感知MMD
    )
    weight_adjuster = AdaptiveBridgeWeight(
        initial_weight=0.05, max_weight=0.3, warmup_steps=50, decay_start=300
    )
    distance_monitor = DomainDistanceMonitor()
    ...
    print("=" * 70)
    print("开始 Step-1: 完整桥梁域增强训练（包含自适应权重和监控）")
    print("=" * 70)
    ...
    while step < 1000:
        # 数据迭代
        if bridge_loader is not None:
            data_zip = zip(source_loader, target_loader, bridge_loader)
        else:
        ...
        for batch_idx, data in enumerate(data_zip):
            # 准备数据
            if bridge_loader is not None:
                (im_source, label_source), (im_target, label_target), (im_bridge, label_bridge) = data
            else:
            ...
            if im_bridge is not None:
                im_bridge = im_bridge.to(device)
                label_bridge = label_bridge.to(device)
            # ========== 前向传播 ==========
            net.train()
            ...
            # 桥梁域
            if im_bridge is not None:
                fb1, feature_bridge, predict_prob_bridge = net.forward(im_bridge)

            # ========== 计算损失 ==========
            # 1. 分类损失
            ce_loss = CrossEntropyLoss(label_source, predict_prob_source)
            if im_bridge is not None:
                ce_bridge = CrossEntropyLoss(label_bridge, predict_prob_bridge)
                ce_loss = ce_loss + 0.5 * ce_bridge
            ...
            if im_bridge is not None:
                coarse_bridge = discriminator_coarse.forward(fb1)
                d1_bridge = BCELossForMultiClassification(label_bridge[:, 0:num_class], coarse_bridge)
                d1_loss = d1_loss + 0.3 * d1_bridge

            # 3. 桥梁域对齐损失（使用自适应权重）
            bridge_alignment_loss = 0
            align_loss_dict = {}
            if im_bridge is not None:
                align_loss, align_loss_dict = bridge_loss_fn(fs1, fb1, ft1)
                current_weight = weight_adjuster.get_weight()
                bridge_alignment_loss = current_weight * align_loss
            ...
            gamma = 0.5  # 判别器损失权重
            # 总损失
            # total_loss = ce_loss + d1_loss + bridge_alignment_loss
            total_loss = alpha * ce_loss + gamma * d1_loss + beta * bridge_alignment_loss
            # ========== 反向传播 ==========
            ...
            if step % monitor_interval == 0:
                ...
                # 4. 监控域间距离
                if step % (monitor_interval * 5) == 0:  # 每100步监控一次
                    distances = distance_monitor.compute_distances(
                        net, source_loader, bridge_loader, target_loader, epoch=step
                    )
                    print(f"域间距离监控 (Step {step}):")
                    if distances:
                ...
                if im_bridge is not None:
                    print(f"  桥梁对齐权重={weight_adjuster.get_weight():.4f}, "
                          f"对齐损失={align_loss_dict}")
    ...
    # 2. 最终域间距离监控
    print("\n最终域间距离分析:")
    final_distances = distance_monitor.compute_distances(
        net, source_loader, bridge_loader, target_loader, epoch=step
    )
    if final_distances:
    ...
    # 3. 计算改进统计
    if len(distance_monitor.distances_history) >= 2:
        initial = distance_monitor.distances_history[0]
        final = distance_monitor.distances_history[-1]

        if 'src_tgt' in initial and 'src_tgt' in final:
            improvement = initial['src_tgt'] - final['src_tgt']
            print(f"\n域间距离改进统计:")
            print(f"  源域-目标域距离减少: {improvement:.4f}")
            print(f"  相对改进: {(improvement / initial['src_tgt'] * 100):.2f}%")
...
def step_2_v2(best_net_path, best_discriminator_path,
    ...
    if bridge_loader is not None:
        print("正在提取桥梁域特征...")
        bridge_feats, bridge_lbls = [], []
        with torch.no_grad():
            for im, lbl in bridge_loader:
                # 修改点9：确保数据在正确的设备上
                im, lbl = im.to(device), lbl.to(device)
                f, _, _ = net(im)
                bridge_feats.append(f.cpu().numpy())
                bridge_lbls.append(lbl.argmax(1).cpu().numpy())
        bridge_feats = np.concatenate(bridge_feats)
        bridge_lbls = np.concatenate(bridge_lbls)
        # 将桥梁域特征与源域特征合并
        src_feats = np.concatenate([src_feats, bridge_feats])
        src_lbls = np.concatenate([src_lbls, bridge_lbls])
    print(f"合并后特征维度: 源域+桥梁域 = {src_feats.shape}")
...
def step_3(source_loader, target_loader, bridge_loader, discriminator, minibatch, net_path, discriminator_fine_path):
    ...
    while k < 40:
        ...
        if bridge_loader is not None:
            for (i, ((im_source, label_source), (im_target, label_target), (im_bridge, label_bridge))) in enumerate(
                    zip(*loaders)):
                net.train()
                discriminator_fine.train()
                discriminator.train()
                ...
                im_bridge, label_bridge = im_bridge.to(device), label_bridge.to(device)
                ...
                domain_prob_discriminator_1_bridge = discriminator.forward(fb1)
                ...
                adv_loss += BCELossForMultiClassification(label=torch.ones_like(domain_prob_discriminator_1_bridge),
                                                          predict_prob=domain_prob_discriminator_1_bridge)
                ...
                with OptimizerManager([optimizer_feature_extractor, optimizer_cls, optimizer_discriminator]):
                    loss = ce + ce_bridge + 0.05 * adv_loss + 0.05 * ce_ep
...
if data_bridge is not None:
    Train_bridge, Test_bridge = Split(data_bridge)
    bridge_loader = Data.DataLoader(dataset=Train_bridge, batch_size=40, shuffle=True, num_workers=4)
    bridge_loader1 = Data.DataLoader(dataset=Test_bridge, batch_size=40, shuffle=True, num_workers=4)
    print(f"桥梁域数据加载完成: 训练集{len(Train_bridge)}样本, 测试集{len(Test_bridge)}样本")
else:
    bridge_loader = None
    bridge_loader1 = None
    print("未使用桥梁域数据")
...
if data_bridge is not None:
    print(f"桥梁域样本数: {len(Train_bridge)}")
...
# Model testing
def find_class_wise_confidence_thresholds(loader, net, num_class, device, min_known_keep=0.98):
    net.eval()
    all_conf, all_labels = [], []
    ...
    with torch.no_grad():
        for im, lbl in loader:
            ...
            conf = prob.max(dim=1)[0].cpu().numpy()

            # 获取真实标签
            if lbl.shape[1] > 1:
                is_unk = lbl[:, -1].cpu().numpy() == 1
                known_labels = lbl[:, :num_class].argmax(dim=1).cpu().numpy()
                batch_labels = np.where(is_unk, num_class, known_labels)
            else:
    ...
    for c in range(num_class):
        mask = all_labels == c
        if mask.sum() < 10:  # 样本太少则用默认
            thresholds.append(0.5)
            continue

        class_conf = all_conf[mask]
        # 找到使至少 min_known_keep 比例的样本不被判为未知的阈值
        # 即：conf >= th 的比例 >= min_known_keep
        thresh = np.percentile(class_conf, (1 - min_known_keep) * 100)
        thresholds.append(max(thresh, 0.1))  # 确保不低于0.1

    return np.array(thresholds)

print("开始模型测试...")
def test_model(loader, net, num_class, class_thresholds):
    net.eval()
    ...
    with torch.no_grad():
        for im, lbl in loader:
            ...
            # 获取预测结果和置信度
            max_conf, pred_known = prob.max(dim=1)
            max_conf_np = max_conf.cpu().numpy()
            pred_known_np = pred_known.cpu().numpy()

            # 为每个样本选择其预测类别对应的阈值
            thresholds = np.array([class_thresholds[p] if p < len(class_thresholds) else class_thresholds[-1]
                                   for p in pred_known_np])

            # 仅置信度判断：低于阈值则为未知类
            is_unknown = max_conf_np < thresholds

            # 最终预测
            preds = np.where(is_unknown, num_class, pred_known_np)

            # 获取真实标签
            if lbl.shape[1] > 1:
                is_unknown_true = lbl[:, -1].cpu().numpy()
                known_labels = lbl[:, :num_class].argmax(dim=1).cpu().numpy()
            ...
            all_confs.append(max_conf_np)

    return (np.concatenate(all_labels) if all_labels else np.array([]),
            np.concatenate(all_preds) if all_preds else np.array([]),
            np.concatenate(all_confs) if all_confs else np.array([]))

#步骤1：计算类别特定置信度阈值
class_conf_thresholds = find_class_wise_confidence_thresholds(
    target_loader1, net, num_class, device, min_known_keep=0.98
)
# print(f"各类别阈值: {np.round(class_conf_thresholds, 4)}")

# 步骤2：用类别阈值进行测试
print("使用类别阈值测试...")
labels, preds, confidences = test_model(
    target_loader1, net, num_class, class_conf_thresholds
)
...
plt.tight_layout()
plt.savefig('./results_noenergy/confusion_matrix_wavelet.png', dpi=300, bbox_inches='tight')
plt.close()
...
print(f"[开放集] 整体准确率: {overall_acc * 100:.1f}%")
# 对源域和目标域的测试进行合并分析
print("模型测试完成！")
...
def visualize_features_confidence_only(net, target_loader, label_names, device=device,
                                       class_thresholds=None):
    """
    仅使用置信度 + 类别特定阈值的特征可视化
    """
    if class_thresholds is None:
        raise ValueError("必须提供类别特定阈值 class_thresholds")
    ...
    with torch.no_grad():
        for x, y in target_loader:
            ...
            # 最终预测
            batch_predictions = np.where(is_unknown, num_class, pred_known_np)

            features.append(f_np)
    ...
    plt.savefig('./results_noenergy/feature_visualization_wavelet.png', dpi=300, bbox_inches='tight')
    plt.close()

# 桥梁域效果评估
def evaluate_bridge_domain_performance(net, source_loader, bridge_loader, target_loader, device=device):
    print("评估桥梁域效果...")
    net.eval()
    # 提取各域特征
    src_features, src_labels = [], []
    if bridge_loader is not None:
        bridge_features, bridge_labels = [], []
    tgt_features, tgt_labels = [], []
    with torch.no_grad():
        # 源域特征
        for im, lbl in source_loader1:
            # 修改点23：确保数据在正确的设备上
            im, lbl = im.to(device), lbl.to(device)
            f, _, _ = net(im)
            src_features.append(f.cpu().numpy())
            src_labels.append(lbl.argmax(1).cpu().numpy())
        # 桥梁域特征
        if bridge_loader is not None:
            for im, lbl in bridge_loader1:
                # 修改点24：确保数据在正确的设备上
                im, lbl = im.to(device), lbl.to(device)
                f, _, _ = net(im)
                bridge_features.append(f.cpu().numpy())
                bridge_labels.append(lbl.argmax(1).cpu().numpy())
        # 目标域特征
        for im, lbl in target_loader1:
            # 修改点25：确保数据在正确的设备上
            im, lbl = im.to(device), lbl.to(device)
            f, _, _ = net(im)
            tgt_features.append(f.cpu().numpy())
            tgt_labels.append(lbl.argmax(1).cpu().numpy())
    src_features = np.concatenate(src_features)
    src_labels = np.concatenate(src_labels)
    if bridge_loader is not None:
        bridge_features = np.concatenate(bridge_features)
        bridge_labels = np.concatenate(bridge_labels)
    tgt_features = np.concatenate(tgt_features)
    tgt_labels = np.concatenate(tgt_labels)

    # 计算域间距离
    def calculate_domain_distance(features1, features2):
        centroid1 = np.mean(features1, axis=0)
        centroid2 = np.mean(features2, axis=0)
        return np.linalg.norm(centroid1 - centroid2)

    if bridge_loader is not None:
        src_bridge_dist = calculate_domain_distance(src_features, bridge_features)
        bridge_tgt_dist = calculate_domain_distance(bridge_features, tgt_features)
        src_tgt_dist = calculate_domain_distance(src_features, tgt_features)

        print(f"域间距离分析:")
        print(f"  源域-桥梁域距离: {src_bridge_dist:.4f}")
        print(f"  桥梁域-目标域距离: {bridge_tgt_dist:.4f}")
        print(f"  源域-目标域距离: {src_tgt_dist:.4f}")
        print(f"  桥梁域减少的域差距: {(src_tgt_dist - (src_bridge_dist + bridge_tgt_dist)):.4f}")

        # 可视化域间关系
        plt.figure(figsize=(8, 6))
        domains = ['Source-Bridge', 'Bridge-Target', 'Source-Target']
        distances = [src_bridge_dist, bridge_tgt_dist, src_tgt_dist]
        colors = ['blue', 'orange', 'red']

        bars = plt.bar(domains, distances, color=colors, alpha=0.7)
        plt.ylabel('Feature Distance', fontsize=12)
        plt.title('Impact of Bridge Domain on Inter-domain Distance', fontsize=14)
        ...
        plt.tight_layout()
        plt.savefig('./results_noenergy/bridge_domain_effect_wavelet.png', dpi=300, bbox_inches='tight')
        plt.close()

        return src_bridge_dist, bridge_tgt_dist, src_tgt_dist
...
# 2. 特征可视化
# 关键：使用测试集
visualize_features_confidence_only(
    net,
    target_loader1,
    Label_Name,
    device=device,
    class_thresholds=class_conf_thresholds  # 传入类别阈值
)

# 3. 桥梁域效果评估
bridge_src_dist, bridge_tgt_dist, src_tgt_dist = evaluate_bridge_domain_performance(net, source_loader, bridge_loader,
                                                                                    target_loader)
