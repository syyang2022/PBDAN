...
# 创建结果目录
os.makedirs("./results_gearbox", exist_ok=True)
...
def get_dat(data_num):
    if data_num == 1:
        # 加载任务1数据
        F_0 = np.load("./gearset/datagearset_thirdspace/T1_source_image.npy")
        F_1 = np.load("./gearset/datagearset_thirdspace/T1_target_image.npy")
        ...
        # 加载桥梁域数据
        try:
            F_bridge = np.load("./gearset/datagearset_thirdspace/T1_bridge_image.npy")
        ...
        # 加载任务2数据
        F_0 = np.load("./gearset/datagearset_thirdspace/T2_source_image.npy")
        F_1 = np.load("./gearset/datagearset_thirdspace/T2_target_image.npy")
        ...
        # 加载桥梁域数据
        try:
            F_bridge = np.load("./gearset/datagearset_thirdspace/T2_bridge_image.npy")
        ...
        # 加载任务3数据
        F_0 = np.load("./gearset/datagearset_thirdspace/T3_source_image.npy")
        F_1 = np.load("./gearset/datagearset_thirdspace/T3_target_image.npy")
        ...
        # 加载桥梁域数据
        try:
            F_bridge = np.load("./gearset/datagearset_thirdspace/T3_bridge_image.npy")
    ...
    # 打印数据形状信息
    print(f"源域数据形状: {F_0.shape}")
    print(f"目标域数据形状: {F_1.shape}")
    print(f"源域标签形状: {Y_0.shape}")
    print(f"目标域标签形状: {Y_1.shape}")

    if F_bridge is not None:
        print(f"桥梁域数据形状: {F_bridge.shape}")
        print(f"桥梁域标签形状: {Y_bridge.shape}")

    # 转换为PyTorch张量
    # 注意: 这里假设数据已经是正确的形状，如果图像数据需要调整通道维度，请取消下面的注释
    F_0 = F_0.transpose((0, 3, 1, 2)) / 255  # 如果需要将(H,W,C)转换为(C,H,W)并归一化
    F_1 = F_1.transpose((0, 3, 1, 2)) / 255
    if F_bridge is not None:
        F_bridge = F_bridge.transpose((0, 3, 1, 2)) / 255
...
class DomainDistanceMonitor:
    ...
    实时监控三个域之间的距离变化
    """
    def __init__(self, log_dir='./results_gearbox/domain_monitor'):
...
plt.tight_layout()
plt.savefig('./results_gearbox/confusion_matrix_wavelet.png', dpi=300, bbox_inches='tight')
plt.close()
...
def visualize_features(net, target_loader, label_names, device=device):
    ...
    plt.savefig('./results_gearbox/feature_visualization_wavelet.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def evaluate_bridge_domain_performance(net, source_loader, bridge_loader, target_loader, device=device):
    ...
    if bridge_loader is not None:
        ...
        plt.tight_layout()
        plt.savefig('./results_gearbox/bridge_domain_effect_wavelet.png', dpi=300, bbox_inches='tight')
        plt.close()
